...
const CardCandidates = ({
  ...
  return (
    ...
    >
      ...
      <div className="card-info">
        <DressIcon />{' '}
        <span className="card-info__text">Master of Computer Science</span>
      </div>
      <div className="card-info">
        <FoundedIcon />{' '}
        <span className="card-info__text">12 years of exp • 27 years old</span>
      </div>
      <div className="card-info">
        <WalletIcon color="#C4C4C4" />{' '}
        <span className="card-info__text">
          Expected {formatMoney(data?.jobseeker?.expected_sallary)} (
          {data?.jobseeker?.expected_sallary_negotiable ? 'Negotiable' : 'Nett'}
          )
        </span>
      </div>
